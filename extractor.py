...
def get_features(document,candidate,candidate_list, isKeyword):
    features = {}
    L=candidate.split(' ')
    features['length'] = len(L)
    if isKeyword:
        keywordsExtracted.append(L)
...
for doc in os.listdir(doc_path):
    ...
    if doc.endswith('txt'):
        document = open(doc_path + doc,mode="r",encoding="utf-8", errors="replace").read()
        #document = open(document + doc, encoding="utf-8", errors="replace")
        ...
        for team in os.listdir(team_path):
            ...
            if team.startswith('team'):
                ...
                keywords = open(team_path + team + '/' + doc_name + '.key',mode="r",encoding="utf-8", errors="replace").read()
               # keywords = open(keywords + team + '/' + doc_name + '.key', encoding="utf-8", errors='replace')
                ...
                print("Phase 1 extraction: ",keyword_list)             
...
print(keywordsExtracted) #result
print('Length of keywords above:',len(keywordsExtracted)) #verification
